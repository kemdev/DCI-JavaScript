function print(...args) {
    console.log(...args)
}

let exerNum = 1

function println(isTrue = false, extra = exerNum) {
    isTrue ? extra = isTrue : exerNum++
        print(`---------------${extra}---------------`)
}

// Create a program that helps a cashier give adequate change to customers. The program should return the amount of notes and coins for the customer's change.


// The program should ask the user for the price of the item and the amount tendered. It should then calculate the change due and display it to the user.

// Example: If the price is €3.75 and the paid amount is €50, then the client should receive €46.25 back in change.

/**
 * The expected output should be:
 * 2 x €20 // 2 twenty euro notes 
 * 1 x €10 // 1 ten euro note
 * 1 x €5 // 1 five euro note 
 * 1 x €1 // 1 euro 
 * 1 x €0.50 // 1 fifty cent coin
 * 1 x €0.2 // 1 twenty cent coin 
 * 1 x €0.1 // 1 ten cent coin
 * 1 x €0.05 // 1 five cent coin
 * 1 x €0.01 // 1 one cent coin
 */

//  Example: Price: €4.50, Paid amount: €20, Change: 15.50

//  Expected output:

//  1 x €10

//  1 x €5

//  1 x €0.5

//  Notes

//  Include outputs for exceptions e.g. price: €4, paid amount: €3.

function cashier(price, paid) {
    let change = paid - price;
    let result = 0;
    let coins = [2, 1, 0.5, 0.2, 0.1, 0.05, 0.01];
    let notes = [20, 10, 5];
    let changeNotes = [];
    let changeCoins = [];
    let changeResult = [];

    if (change < 0) {
        return "Insufficient amount!";
    }


    for (let i = 0; i < notes.length; i++) {
        let note = notes[i];
        let noteCount = Math.floor(change / note);
        change = change % note;
        changeNotes.push(noteCount);

        if (noteCount > 0) {
            changeResult.push(`${noteCount} x €${note}`);
        }
    }

    for (let i = 0; i < coins.length; i++) {
        let coin = coins[i];
        let coinCount = Math.floor(change / coin);
        change = change % coin;
        changeCoins.push(coinCount);

        if (coinCount > 0) {
            changeResult.push(`${coinCount} x €${coin}`);
        }


    }
    return changeResult.join("\n");
}


print(cashier(4.50, 20))